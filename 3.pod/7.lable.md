...
在前面的学习中，我们学到了 Deployment 部署，以及副本数\(ReplicaSet\)，但是 Pod 部署到哪个 Worker 节点是随机的，即使有 3个 Woker 和设定 3个 副本，不一定每个 Node 刚刚好运行一个 Pod，也可能其中 Node 运行着三个副本。
...
> 使用 `kubectl get pod {名称} -o yaml` 查询，或者使用 `kubectl edit pod {名称}` 查看修改 Pod 的 表标签。
...
或者 kubectl get pods -l a,b ，包含这两种标签的 pod(不理会值)

列出不包含 env 标签的 pod ：`kubectl get pods -l '!env'`
...
YAML 只支持 `{key}:{value}` 这种形式，也就是说只能使用 `&&`，多个条件同时满足。而我们使用命令形式进行查询时，则可使用以上三种运算符。
...
我们要部署一个 pod，在 big、medium 中都可以运行(相对于 `||`)，则：
...
> 目前提到的标签选择，主要是命令式选择，比较容易使用，而且支持的选择方式比较多；但是写进 YAML 或 JSON 中的选择器，则编写比较复杂，而且不支持所有运算符。
...
在之前的创建的 Deployment(nginx) ，其 Pod 都会带上 `app=nginx` 的标签，我们可以在创建 Pod 之前，设置 Pod 的标签。
