...
Service 为 Pod 提供了一种外网访问能力，默认情况下，Pod 只能在 Kubernetes 集群的**同一节点访问**（TODO），如果要外部网络访问，则需要为 Pod 暴露一个 Kubnetes Service，Service 为 Pod 提供了外网访问能力。这里我们把上一小节的 hello 暴露出去。
