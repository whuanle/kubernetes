...
我们也可以手动配置，执行 `kubectl version` 查看版本号，找到 `GitVersion:v1.21.1` ，下载 yaml 文件 `https://cloud.weave.works/k8s/net?k8s-version=v1.21.1`。然后执行 `kubectl apply -n kube-system -f net.yaml` 即可。
